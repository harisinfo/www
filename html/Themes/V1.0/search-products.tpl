<div id="products">
	...
	<div id="product_{$key}">
		<h2><a href="?module=product&category={$request.search.category_id}&sub_category_id={$request.search.sub_category_id}&product_id={$key}">{$response.product.product_title.$key}</a></h2> 
		...
		{foreach key=key1 item=item1 from=$response.product.brand.$key}
			...
			{if $response.product.combine_product_id.$key.$key1==0}
				...
				{if $response.product.medical_condition_id.$key}
					<a href="?module=condition&condition_id={$response.product.medical_condition_id.$key}&product_id={$response.product.product_id.$key}">Start Free Consultation to Order</a>
				{else}
